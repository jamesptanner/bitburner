...
export const isInstanceOf =
  <T>(ctor: new (...args: unknown[]) => T) =>
  (x: unknown): x is T =>
    x instanceof ctor;

export function is2DArray<T>(
  val: unknown,
  elementGuard: (x: unknown) => x is T,
): val is T[][] {
  return (
    Array.isArray(val) &&
    val.every((va) => Array.isArray(va) && va.every(elementGuard))
  );
}

export const unique = <T>(v: T, i: number, self: T[]): boolean => {
...
export async function walk(
  ns: NS,
  start: string,
  func: (host: string | undefined) => Promise<boolean>,
): Promise<void> {
...
export async function cacheAllServers(ns: NS): Promise<string[]> {
  ...
  allHosts = allHosts.filter((v, i, self) => {
    return (
      self.indexOf(v) === i && v !== "home" && !ns.fileExists("ignore.txt", v)
    );
  });
...
export const findBestTarget = function (ns: NS): string {
  ...
  getAllServers(ns).forEach((server) => {
    ...
    if (
      serverDetails &&
      serverDetails.backdoorInstalled &&
      serverDetails.moneyMax &&
      serverDetails.moneyMax > maxFunds
    ) {
      bestServer = server;
...
export const canUseSingularity = function (ns: NS): boolean {
  return (
    ns.singularity.getOwnedSourceFiles().filter((x) => {
      return x.n === 4;
    }).length !== 0
  );
};
...
export const getConstServerInfo = function (
  ns: NS,
  host: string,
): ServerInfo | undefined {
  const servers = new Map<string, ServerInfo>(
    JSON.parse(ns.read("servers.txt") as string),
  );
  return servers.get(host);
};

export const routeToHost = function (
  ns: NS,
  start: string,
  end: string,
): string[] {
...
export const needToFocus = function (ns: NS): boolean {
  if (
    ns.singularity
      ...
      .indexOf("Neuroreceptor Management Implant") !== -1
  )
    return false;
  return true;
};
