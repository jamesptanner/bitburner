 ...
 */
const purchaseAugment = async function (
  ns: NS,
  faction: string,
  augment: string,
) {
  ...
  while (
    ...
    while (
      ns.getPlayer().money < ns.singularity.getAugmentationPrice(augment)
    ) {
      const currentMoneyCheck = ns.getPlayer().money;
      const moneyDiff = currentMoneyCheck - lastMoneyCheck;
      logging.info(
        `estimated time remaining: ${ns.tFormat((ns.singularity.getAugmentationPrice(augment) - currentMoneyCheck) / ((60 * 1000) / moneyDiff))}`,
      );
  ...
  if (
    purchaseAttempt === 3 &&
    ns.singularity.getOwnedAugmentations(true).indexOf(augment) === -1
  ) {
...
const purchaseAugments = async function (
  ns: NS,
  faction: string,
  augments: string[],
) {
  ...
  const sortedAugments = augments.sort((a, b) => {
    ...
    ); //prices change but the order wont.
  });
  for (const augment of sortedAugments) {
    ...
    if (
      ...
      await improveFactionReputation(
        ns,
        faction,
        ns.singularity.getAugmentationRepReq(augment),
      );
    ...
    if (ns.singularity.getAugmentationPrereq(augment).length > 0) {
      //handle the augment pre requirements first.
      logging.warning(`getting prerequisite for ${augment} first`);
      const unownedPrerequisites = ns.singularity
        .getAugmentationPrereq(augment)
        .filter((preReq) => {
          return (
            ns.singularity.getOwnedAugmentations(true).indexOf(preReq) === -1
          );
        });
...
async function buyExistingAugments(ns: NS, availableAugments: string[]) {
  ...
  const factionForAugment = availableAugments
    .map((augment) => {
    ...
    .sort((a, b) => {
      ...
      return (
        ...
        ns.singularity.getAugmentationPrice(b[0])
      );
    })
    .reverse();
