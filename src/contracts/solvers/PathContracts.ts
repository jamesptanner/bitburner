...
export function MinTrianglePath(
  ...
  if (
    ...
    for (let row = 1; row < numberArray.length; row++) {
      for (let col = 0; col < numberArray[row].length; col++) {
        if (col === 0) {
          ...
          numberArray[row][col] += Math.min(
            numberArray[row - 1][col],
            numberArray[row - 1][col - 1],
          );
        }
      }
    }
    ...
    logging.success(
      `MinPath: ${Math.min(...numberArray[numberArray.length - 1])}`,
    );
...
export function colorGraph(
  ...
  if (
    ...
    is2DArray<number>(data[1], (val: unknown): val is number => {
      return typeof val === "number";
    })
  ) {
    const nodeCount = data[0];
    const edges = data[1].sort((a, b) => {
      return a[0] - b[0];
    });
    ...
    while (
      ...
      for (let index = 0; index < out.length; index++) {
        const matchingEdges = edges.filter((v) => {
          return v[0] === index;
        });
        if (out[index] === -1) {
          //do we have a value for any of the opposite?
          const usableEdges = matchingEdges.filter((v) => {
            return v[1] !== -1;
          });
